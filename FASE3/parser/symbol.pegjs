...
directive_section
    ...
    {if(exp){
        if(exp instanceof Array){exp.forEach(e => {if(e !== null){e.environment = d.toString().replaceAll(/\s/g, '').replaceAll(',', ''); symbols.push(e);}});
        } else {if(exp !== null){ exp.environment = d.toString().replaceAll(/\s/g, '').replaceAll(',', ''); symbols.push(exp);}}
    }}
directive_exp
    = e:directive
...
// ************************************************** Expresiones ************************************************** \\
expression
    = i:identifier e:equal v:value 
    {return new Symbol(i,'constant', v, '', location().start.line, location().start.column);}
    / l:label _* "\n"? d:directive _* v:value vl:((comma value)*)  _* comment? "\n"? 
    {if(vl.length){let array = []; array.push(v); vl.forEach(value => {if(value !== null){array.push(value[1]);}});
        return new Symbol(l.toString().replaceAll(/\s/g, '').replaceAll(',', '').replaceAll(':', ''), d.toString().replaceAll(/\s/g, '').replaceAll(',', ''),  array, '', location().start.line, location().start.column);
    }else{
        return new Symbol(l.toString().replaceAll(/\s/g, '').replaceAll(',', '').replaceAll(':', ''), d.toString().replaceAll(/\s/g, '').replaceAll(',', ''), v, '', location().start.line, location().start.column);}
    }
...
identifier "Identificador"
    = _* [a-zA-Z_] [a-zA-Z0-9_]* {let v = new Value(TYPE.CONSTANT, text().replaceAll(/\s/g, ''), text().replaceAll(/\s/g, '')); return v;}
// ************************************************** String ************************************************** \\
string "String"
    = _* "\"" [^"]* "\"" {let v = new Value(TYPE.STRING, '', text().slice(1,-1)); v.convertToType(); return v;}
// ************************************************** Char ************************************************** \\
char "Char"
    = _* "'" [^'] "'" {let v = new Value(TYPE.CHAR, '', text().slice(1,-1)); v.convertToType(); return v;}
// ************************************************** Octal ************************************************** \\
oct "Octal"
    = _* "0" [0-7]+ {let v = new Value(TYPE.OCT, '', text().replaceAll(/\s/g, '')); v.convertToType(); return v;}
// ************************************************** Hexadecimal ************************************************** \\
hex "Hexadecimal"
    = _* "0x" [0-9a-fA-F]+ {let v = new Value(TYPE.HEX, '', text().replaceAll(/\s/g, '')); v.convertToType(); return v;}
// ************************************************** Binario ************************************************** \\
bin "Binario"
    = _* "0b" [01]+ {let v = new Value(TYPE.BIN, '', text().replaceAll(/\s/g, '')); v.convertToType(); return v;}
// ************************************************** Entero ************************************************** \\
int "Entero"
    = _* ("+"/"-")? [0-9]+ {let v = new Value(TYPE.INT, '', text().replaceAll(/\s/g, '')); v.convertToType(); return v;}
// ************************************************** Línea en blanco ************************************************** \\
