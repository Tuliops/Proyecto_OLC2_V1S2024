{
    const TYPE = {
        EMPTY: 'Vacio',
        COMMA: 'Coma',
        EXCLAMATION: 'Signo de Exclamacion',
        RB: 'Llave Derecha',
        LB: 'Llave Izquierda',
        PLUS: 'Mas',
        Y: 'Simbolo &',
        ...
        XOR: 'Simbolo ⊕',
        COMMENT: 'Comentario',
        BLANK: 'Linea en Blanco',
        INTEGER: 'Numero Entero',
        BIN: 'Numero Binario',
        HEX: 'Numero Hexadecimal',
        OCTAL: 'Numero Octal',
        ...
        SIGN: 'Signo',
        IMM: 'Inmediato',
        Q: 'Sufijo de Condicion',
        CC: 'Codigo de Condicion',
        SYSREG: 'Registro de Sistema',
        SY: 'Sufijo de Sistema',
        R64: 'Registro de 64 bits',
        R32: 'Registro de 32 bits',
        SP64: 'Puntero de Pila de 64 bits',
        SP32: 'Puntero de Pila de 32 bits',
        ZR64: 'Registro Cero de 64 bits',
        ZR32: 'Registro Cero de 32 bits',
        PC: 'Contador de Programa',
        LR: 'Registro de Enlace',
        FP: 'Registro de Marco',
        EXTEND: 'Extendido',
        OPEXTEND: 'Operando Extendido',
        SHIFT: 'Desplazamiento',
        OPSHIFT: 'Operando de Desplazamiento',
        BARRIEROP: 'Operacion de Barrera',
        PRFOP: 'Operacion de Prefetch',
        ADDR: 'Direccion de Memoria',
        SH: 'Sufijo de Desplazamiento',
        MASK: 'Mascara',
        REL: 'Etiqueta de Salto',
        OP2A: 'Operando 2 Aritmetico',
        OP2L: 'Operando 2 Logico',
        INST: 'Instruccion',
        OP: 'Operador',
        ARGS: 'Argumentos',
        DIRECTIVE_SECTION: 'Seccion de Directivas',
        DIRECT_EXP: 'Expresion de Directiva',
        INSTRUCTION_SECTION: 'Seccion de Instrucciones',
    ...
    class CSTnode{
        ...
        addChildren_Values(children){
            children.forEach(child => {
                //check if the child is a js list and add all the children
        ...
        getDot(){
            ...
            dot += 'digraph G {\n   splines=polyline;\n nodesep=0.1;\n  ranksep=0.5;\n  pack=true;\n';
            function addNodes(n){
                if(n.type === TYPE.ROOT){dot += `   ${n.id} [label="${n.value}", shape=tripleoctagon, style=filled, color=lightblue];\n`;}
                else if(equalType(n.type, TYPE.INST, TYPE.DIRECTIVE_SECTION, TYPE.INSTRUCTION_SECTION, TYPE.INSTRUCTIONS, TYPE.EXPRESSION, TYPE.DIRECT_EXP))
                ...
                else{dot += `   ${n.id} [label="${n.value}"];\n`;}
                n.children.forEach(child => {
                    if(equalType(n.type, TYPE.DIRECTIVE_SECTION, TYPE.DIRECT_EXP, TYPE.EXPRESION, TYPE.INSTRUCTIONS)){dot += `    ${n.id} -> ${child.id}[weight=3];\n`;}
                    else{dot += `    ${n.id} -> ${child.id};\n`;}
                    addNodes(child);
                });
            }
            addNodes(this);
        ...
        getINSnodes(){
            ...
            function getInstructions(n){
                if(n.type === TYPE.INST){ins.push(n);}
        ...
        getINSTnodes(){
            let ins = [];
            function getInstructions(n){
                if(n.type === TYPE.INSTRUCTION_SECTION){ins.push(n.children[0]);}
                else if(n.type === TYPE.INST){ins.push(n);}
                n.children.forEach(child => getInstructions(child));
            }
            getInstructions(this);
            return ins;
        ...
        Text(){
            let text = 'EJECUCION EXITOSA\n*******************\nSe han encontrado las siguientes instrucciones:\n*******************\n';
            function getInstructions(n){
                if(n.type === TYPE.INSTRUCTION_SECTION){text += `${n.children[0].value}\n`;}
    ...
    function createArgsNode(children = []){
        return new CSTnode(TYPE.ARGS, 'args', '', children);
    }
...
expression
    ...
    {const n = createNode(TYPE.EXPRESSION, 'EXPRESSION', ''); n.addChild(l); n.addChild(d); n.addChild(s); return n;}
...
ret_inst
    ...
    {const n = createNode(TYPE.OP, 'op', ''); n.addChild(op); if(c){n.addChild(c);} if(q){n.addChild(q)} const ins = createInstNode('BRANCH INSTRUCTION'); ins.addChild(n);
    if(a){const arg = createNode(TYPE.ARGS, 'args', ''); arg.addChild(a); ins.addChild(arg);} return ins;}
RET
...
iint "Inmediato Entero"
    = _* h:"#"i? s:("+"/"-")? v:int {const n = createNode(TYPE.IMM, '#imm', ''); if(h){const nh = createNode(TYPE.NUMERAL, h, ''); n.addChild(nh);} if(s){const ns = createNode(TYPE.SIGN, s, ''); n.addChild(ns);} n.addChild(v); return n;}
...
// ************************************************** String ************************************************** \\
string "String"
    = _* "\"" [^"]* "\"" {return createNode(TYPE.STRING, text().slice(1,-1), '');}
// ************************************************** Char ************************************************** \\
char "Char"
    = _* "'" [^'] "'" {return createNode(TYPE.CHAR, text().slice(1,-1), '');}
// ************************************************** Octal ************************************************** \\
oct "Octal"
    = _* "0" [0-7]+ {return createNode(TYPE.OCTAL, text().replaceAll(/\s/g, ''), '');}
// ************************************************** Hexadecimal ************************************************** \\
hex "Hexadecimal"
    = _* "0x" [0-9a-fA-F]+ {return createNode(TYPE.HEX, text().replaceAll(/\s/g, ''), '');}
// ************************************************** Binario ************************************************** \\
bin "Binario"
    = _* "0b" [01]+ {return createNode(TYPE.BIN, text().replaceAll(/\s/g, ''), '');}
// ************************************************** Entero ************************************************** \\
int "Entero"
    = _* [0-9]+ {return createNode(TYPE.INTEGER, text().replaceAll(/\s/g, ''), '');}
// ************************************************** Línea en blanco ************************************************** \\
